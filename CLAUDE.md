...
There are two branches: staging and master. Development is done on staging. Staging is connected to Netlify for deployment. **Master deploys to GitHub Pages *and* Cloudflare Pages at the same time** — a deliberate, temporary state while hosting migrates; `www.bandincc.it` still resolves to GitHub Pages, and `bandincc.pages.dev` is the same tested artifact on the new host. The end state is Cloudflare only, with a private repo and no Netlify: the ordered steps, what is done and what is left, are in `roadmap.md` → "Going private on Cloudflare Pages". **Read that before changing anything about deployment.** Note `*.pages.dev` is unreachable from at least one Italian ISP — DNS resolves but both 80 and 443 hang, even pinned to Cloudflare's IPs — so verify it in a browser, not with curl from the dev machine. The domain is www.bandincc.it
